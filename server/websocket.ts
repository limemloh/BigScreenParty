import { Stream, producerStream, Reactive, 
  Behavior, producerBehavior, performStream, 
  changes, Now, isBehavior, isStream, placeholder, sinkBehavior, sinkStream} from "@funkia/hareactive";
import { withEffects } from "@funkia/jabz";

export const subscribe = (socket: WebSocket) => producerStream<string>(push => {
  socket.onmessage = ({data}) => push(data);
  return () => socket.onmessage = () => {};
});

export const send = withEffects((socket: WebSocket, message: string) => {
  socket.send(message);
});

type Reactives = Record<string, Reactive<string>>
export function loopSocket(reactives: Reactives, socket: WebSocket, names?: string[]): Reactives {
  for(const name of Object.keys(reactives)) {
    const reactive = reactives[name];
    reactive.subscribe(value => socket.send(JSON.stringify({
      type: isBehavior(reactive) ? "Behavior" : "Stream", 
      name, 
      value
    })));
  }

  const created: any = {};
  const p = names === undefined ? new Proxy({}, {
    get: function (target: any, name: string) {
      if (name in created) {
        return created[name];
      }
      if (!(name in target)) {
        target[name] = placeholder();
      }
      return target[name];
    }
  }) : names.reduce((obj: any, name: string) => {
    obj[name] = placeholder();
    return obj;
  }, {});
  
  socket.onmessage = ({data}) => {
    const {name, type, value} = JSON.parse(data);
    if (!(name in created)) {
      const replacement = type === "Behavior" ? sinkBehavior("") : sinkStream();
      if (name in p) {
        p[name].replaceWith(replacement);
      }
      created[name] = replacement;
    }
    created[name].push(value);
  }
  return p;
}



export function sendBehavior(socket: WebSocket, name: string,  behavior: Behavior<string>): Now<any> {  
  const sends = changes(behavior).map(value => send(socket, JSON.stringify({type: "Behavior", name, value})));
  return performStream(sends);
}

export function receiveBehavior(socket: WebSocket, name: string, initial: string): Behavior<string> {
  return producerBehavior(push => {
    socket.onmessage = ({data}) => {
      const desc = JSON.parse(data);
      if (desc.name === name) {
        push(desc.value);
      }
    };
    return () => socket.onmessage = () => {};  
  }, initial);
}
